...
static void constructor(void *ptr, va_list *args)
{
    ...
    self->prevMousePos = (sfVector2i){self->parent->screenWidth / 2, self->parent->screenHeight / 2};
}
...
{
    sfRenderWindow_setMouseCursorVisible(self->parent->render->window, sfFalse);
    sfWindow_setMouseCursorGrabbed((sfWindow *)self->parent->render->window, sfTrue);
...
void mouse_rot(InputClass_t *self)
{
    ...
    if (self->mouseDistance > self->mouseThreshold) {
        self->rotate = 1;
    } else if (self->mouseDistance < -self->mouseThreshold) {
        self->rotate = -1;
    }
