...
void create_minimap_background(HUDClass_t *self)
{
    ...
    sfRectangleShape_setSize(self->minimap_background,
        (sfVector2f){MINIMAP_SIZE, MINIMAP_SIZE});
    sfRectangleShape_setPosition(self->minimap_background,
        self->minimap_position);
    sfRectangleShape_setFillColor(self->minimap_background,
        (sfColor){30, 30, 30, 200});
...
void render_minimap(HUDClass_t *self)
{
    ...
    for (i = 0; i < self->minimap_wall_count; i++) {
        if (self->minimap_walls[i])
            sfRenderWindow_drawSprite(self->parent->render->window,
                self->minimap_walls[i], NULL);
    }
    if (self->minimap_player)
        sfRenderWindow_drawCircleShape(self->parent->render->window,
            self->minimap_player, NULL);
    if (self->minimap_direction)
        sfRenderWindow_drawVertexArray(self->parent->render->window,
