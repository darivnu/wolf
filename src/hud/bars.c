...
void create_health_bar(HUDClass_t *self)
{
    ...
    sfRectangleShape_setSize(
        self->health_bar, (sfVector2f){self->max_bar_width, BAR_SIZE});
    sfRectangleShape_setPosition(
        self->health_bar, (sfVector2f){BAR_X, HEALTH_BAR_Y});
...
void create_mana_bar(HUDClass_t *self)
{
    ...
    sfRectangleShape_setSize(
        self->mana_bar, (sfVector2f){self->max_bar_width, BAR_SIZE});
    sfRectangleShape_setPosition(
        self->mana_bar, (sfVector2f){BAR_X, MANA_BAR_Y});
...
void update_health_bar(HUDClass_t *self, float percentage)
{
    ...
    sfRectangleShape_setSize(
        self->health_bar, (sfVector2f){new_width, BAR_SIZE});
}
...
void update_mana_bar(HUDClass_t *self, float percentage)
{
    ...
    new_width = (self->max_bar_width * percentage) / 100.0f;
    sfRectangleShape_setSize(
        self->mana_bar, (sfVector2f){new_width, BAR_SIZE});
}
