...
void create_wall_sprites(HUDClass_t *self)
{
    ...
    for (y = 0; y < self->parent->map->height; y++) {
        for (x = 0; x < self->parent->map->width; x++) {
            self->process_wall_cell(self, x, y, wall_scale, &wall_index);
        }
    }
}

void process_wall_cell(HUDClass_t *self, int x, int y,
    sfVector2f wall_scale, int *wall_index)
{
    int wall_type = 0;

    wall_type = self->parent->map->map_get_cell(self->parent->map, x, y);
    if (wall_type <= 0)
        return;
    self->minimap_walls[*wall_index] = sfSprite_create();
    self->setup_wall_sprite_texture(self, *wall_index, wall_type);
    self->setup_wall_sprite_position(self, *wall_index, x, y, wall_scale);
    (*wall_index)++;
}
